...
const AllProduct =({searchParams})=> {
	...
	const fillCategory=(cat_id, catName)=>{
		...
		const filterCategory = currentItems.filter((item) => item.category_id == cat_id)
		setCheckCategory(true);
	...
	const fillProductType=( proType_id, proTypeName )=>{
		...
		const filterCategory = currentItems.filter((item) => item.product_type_id == proType_id)
		setCheckCategory(true);
		setAllProduct(filterCategory);
	}
									...
									<div key={items.id} className="w-6/12 md:w-4/12 lg:w-3/12 xl:w-2/12">
										<div className="p-3">
											<div className="bg-white shadow-md hover:shadow-2xl rounded-lg p-3 text-center">
												...
												<Link href={`/single?slug=${items.slug}`}>

														<div className="h-[150px] w-full cursor-pointer">
												...
												<Link href={`/single?slug=${items.slug}`}>
													<span className="cursor-pointer" onClick={()=>showProduct(items.slug)}>
														<h6 className="hover:text-[#d4983d] text-sm font-bold pt-3">{`${items.name.substring(0, 15)}`}</h6>
														<p className="text-gray-700 ">{`${items.composition.substring(0, 19)}...`}</p>
